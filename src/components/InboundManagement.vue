...
const saveInboundRecord = async () => {
  try {
    ...
    MaterialDB.addInboundRecord(currentRecord.value)
...
const exportRecords = () => {
  try {
    ...
    downloadCSV(csvContent, `入库记录_${new Date().toISOString().split('T')[0]}.csv`)
...
const generateCSV = (data) => {
  const headers = ['物资名称', '数量', '单位', '单价', '总价', '供应商', '批次号', '操作员', '入库时间', '备注']
  const rows = data.map(record => [
    record.material_name || '',
    record.quantity,
    record.unit || '',
    (Number(record.unit_price) || 0).toFixed(2),
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="saveInboundRecord">保存</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
